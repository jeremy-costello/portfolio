...
import { Close as CloseIcon, OpenInNew as OpenInNewIcon, Article as ArticleIcon } from '@mui/icons-material';
...
const MediaContainer = styled(Box)(({ theme }) => ({
  ...
  overflow: 'hidden',
  '& video': { width: '100%', borderRadius: theme.spacing(1) },
  '& img': { width: '100%', borderRadius: theme.spacing(1) }
}));
      ...
      <DialogContent sx={{ p: 3 }}>
        {project.mediaType === "component" ? (
          <Box sx={{ width: '100%', borderRadius: 1, overflow: 'hidden' }}>
            <Typography variant="body1" sx={{ color: 'grey.300', mb: 3, lineHeight: 1.0, fontWeight: 'bold', fontSize: '1.25rem' }}>
              {"Try interacting with the demo below!"}
            </Typography>
            <Typography variant="body1" sx={{ color: 'grey.300', mb: 3, lineHeight: 1.0 }}>
              {"Left click to pan, middle click to rotate, mouse wheel (or right click) to zoom."}
            </Typography>
            <CesiumViewer />
        ...
        <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 2, mb: 3 }}>
          ...
          {!project.demoLink && !project.codeLink && !project.pubLink && (
            <Typography variant="body2" sx={{ color: 'grey.400', fontStyle: 'italic' }}>
              {project.demoNote || project.codeNote || project.pubNote || 'Demo/Code/Publication unavailable'}
            </Typography>
          )}
        </Box>
      </DialogContent>
